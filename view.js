...
export class View extends HTMLElement {
  ...
  /**
   * 判断两个 Range 是否交叉或包含
   * @param {Range} range1 第一个 Range 对象
   * @param {Range} range2 第二个 Range 对象
   * @returns {boolean} 如果两个 Range 交叉或包含则返回 true，否则返回 false
   */
  #rangesIntersectOrContain(range1, range2) {
    // 比较两个 Range 的边界点
    const start1 = range1.startContainer;
    const start1Offset = range1.startOffset;
    const end1 = range1.endContainer;
    const end1Offset = range1.endOffset;

    const start2 = range2.startContainer;
    const start2Offset = range2.startOffset;
    const end2 = range2.endContainer;
    const end2Offset = range2.endOffset;

    // 比较函数：判断节点位置关系
    function comparePositions(node1, offset1, node2, offset2) {
      // 如果节点相同，比较偏移量
      if (node1 === node2) {
        return offset1 - offset2;
      }

      // 检查 node1 是否在 node2 之前
    ...
    // 检查 range1 是否完全在 range2 之前
    const range1BeforeRange2 =
      comparePositions(end1, end1Offset, start2, start2Offset) < 0;

    // 检查 range1 是否完全在 range2 之后
    const range1AfterRange2 =
      comparePositions(start1, start1Offset, end2, end2Offset) > 0;

    // 如果 range1 完全在 range2 之前或之后，则不相交
    if (range1BeforeRange2 || range1AfterRange2) {
      return false;
    }

    // 否则，两个 Range 相交或一个包含另一个
    return true;
  }
  ...
  isCfiInVisibleRange(cfi) {
    const { index, anchor } = this.resolveNavigation(cfi);
    const lastVisibleRange = this.renderer.lastVisibleRange;
    if (!lastVisibleRange) return false;
    const contents = this.renderer.getContents();
    if (contents.length === 0) return false;
    const { doc } = contents.find((x) => x.index === index);
    if (!doc) return false;
    const range = anchor(doc);
    return this.#rangesIntersectOrContain(lastVisibleRange, range);
  }
